...
create table FM.transition_terms (
    ...
    topic             FM.topic        not null,
    focus             FM.focus        not null,
    predicate         FM.predicate    not null  default 'true'::FM.predicate,
...
create table FM.transition_phrasids (
    ...
    csqt_clausid      bigint  not null references FM.transition_clausids ( clausid ),
  unique ( cond_clausid, csqt_clausid ) );
...
create view FM.transition_phrases as ( select
    ...
    csqt.termid         as csqt_termid,
    csqt.clausid        as csqt_clausid,
    cond.topic          as cond_topic,
    cond.focus          as cond_focus,
    cond.predicate      as cond_predicate,
    csqt.topic          as csqt_topic,
    csqt.focus          as csqt_focus,
    csqt.predicate      as csqt_predicate
    -- cond.comment       as cond_comment,
    -- csqt.comment        as csqt_comment
  from FM.transition_clauses       as cond
  join FM.transition_phrasids      as phrase on ( cond.clausid         = phrase.cond_clausid )
  left join FM.transition_clauses  as csqt   on ( phrase.csqt_clausid  = csqt.clausid )
  order by cond_clausid, cond_termid );

-- ---------------------------------------------------------------------------------------------------------
create function FM._get_transition_clause_kind( cond_phrasid bigint, csqt_phrasid bigint )
...
create view FM.transition_clauses_and_phrases as ( select
    ...
    csqt_phrase.phrasid   as csqt_phrasid,
  ...
  left join FM.transition_phrasids             as csqt_phrase on ( IDS.clausid = csqt_phrase.csqt_clausid ),
  lateral coalesce( cond_phrase.phrasid, csqt_phrase.phrasid ) as p ( phrasid ),
  lateral FM._get_transition_clause_kind(
    cond_phrase.phrasid,
    csqt_phrase.phrasid )                     as ck ( kind ) );
...
-- ---------------------------------------------------------------------------------------------------------
create function FM.add_transition_term( ¶topic FM.topic, ¶focus FM.focus, ¶predicate FM.predicate )
...
comment on function FM.add_transition_term( FM.topic, FM.focus, FM.predicate ) is 'Given a topic, a focus
...
create function FM.add_transition(
  ¶cond_topic FM.topic, ¶cond_focus FM.focus, ¶cond_predicate FM.predicate,
  ¶csqt_topic FM.topic, ¶csqt_focus FM.focus, ¶csqt_predicate FM.predicate )
...
-- ---------------------------------------------------------------------------------------------------------
\echo :signal ———{ :filename 10 }———:reset
insert into FM.transition_phrasids ( phrasid, cond_clausid, csqt_clausid ) values
  ( 1, 1, 2 );
...
create table FM.transitions_and_actions (
    ...
    csqt_termid     bigint  not null references FM.transition_terms ( termid ),
  primary key ( cond_termid, csqt_termid ) );
...
create view FM.transitions as ( select
    ...
    TP.csqt_termid   as TP_csqt_termid,
    TA.cond_termid  as TA_cond_termid,
    TA.csqt_termid   as TA_csqt_termid
  from FM.transition_terms          as TE
  left join FM.transitions_and_actions as TP on ( TE.termid = TP.cond_termid )
  left join FM.transitions_and_actions as TA on ( TE.termid = TA.csqt_termid )
  -- order by clausid, id
  );
