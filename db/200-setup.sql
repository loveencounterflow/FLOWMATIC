...
create table FM.journal (
    ...
    status      text,
  foreign key ( pjid  ) references FM.journal ( jid ),
...
-- ---------------------------------------------------------------------------------------------------------
create function FM_FSM.queue_moves( ¶parent_event FM.queue )
  returns void volatile language plpgsql as $$
  declare
    ¶row record;
  begin
    perform log( '^388799^', 'parent event', ¶parent_event::text );
