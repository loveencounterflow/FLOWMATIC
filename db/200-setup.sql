...
create table FM.statejournal (
    stateid     bigint generated by default as identity primary key,
    t           timestamp with time zone    not null  default statement_timestamp(),
...
create view FM.current_state as ( select
    ...
    state         as state,
    status        as status
  from FM.statejournal where stateid in ( select distinct
      max( stateid ) over ( partition by topic ) as stateid
...
create view FM.current_state_and_event as (
  ( select null as id, null as t, null as topic, null as focus, null as state, null as kind, null as status where false ) union all
  ( select stateid, t, topic, focus, state, 'state', status from FM.current_state ) union all
  ( select queueid, t, null,   null, event, 'event', null   from FM.current_event )
...
create view FM.journal as (
  ( select null as id, null as t, null as topic, null as focus, null as state, null as kind, null as status where false ) union all
  ( select stateid, t, topic, focus, state, 'state', status from FM.statejournal ) union all
  ( select eventid, t, null,   null, event, 'event', status from FM.eventjournal )
  order by topic, focus );
...
create function FM_FSM.reset() returns void volatile language plpgsql as $$
  ...
  begin
    insert into FM.statejournal ( topic, focus, state,          status )
      select                      topic, focus, topic || focus, 'reset'
      from FM.pairs
      where dflt;
    -- ### TAINT consider to actually use entries in `_transition_phrases`:
    insert into FM.statejournal ( topic,  focus,      state,          status  ) values
...
--   begin
--     insert into FM.journal ( topic, focus, status ) values ( ¶row.topic, ¶row.focus, 'UNPROCESSED' );
--     end; $$;

-- ---------------------------------------------------------------------------------------------------------
create function FM_FSM.move_event_from_queue_to_eventjournal( ¶row FM.queue, ¶status text )
  returns void volatile language plpgsql as $$
  begin
    delete from FM.queue where queueid = ¶row.queueid;
    insert into FM.eventjournal ( event, status ) values ( ¶row.event, ¶status );
    end; $$;
...
create function FM_FSM.apply_current_effects( ¶row FM.queue )
  ...
  begin
    ...
    for ¶effect in select * from FM._current_transition_effects loop
      ...
      insert into FM.statejournal ( topic, focus, state, status )
        values ( ¶effect.topic, ¶effect.focus, ¶effect.state, ¶status );
      end loop;
...
create function FM.process_current_event() returns void language plpgsql as $$
  ...
  begin
    ...
    perform FM_FSM.move_event_from_queue_to_eventjournal( ¶row, ¶status );
    -- .....................................................................................................
    end; $$;
