...
-- PATH SPLITTING
-- ---------------------------------------------------------------------------------------------------------
create function FMAT.split_path( ¶path text ) returns text[] immutable strict parallel safe language sql as $$
  select echo( '^33376^', regexp_match( ¶path, '^([^#]+)#(.+)$' ) ); $$;

-- ---------------------------------------------------------------------------------------------------------
create function FMAT.topic_from_path( ¶path text ) returns text immutable strict parallel safe language sql as $$
  select ( FMAT.split_path( ¶path ) )[ 1 ]; $$;

-- ---------------------------------------------------------------------------------------------------------
create function FMAT.focus_from_path( ¶path text ) returns text immutable strict parallel safe language sql as $$
  select ( FMAT.split_path( ¶path ) )[ 2 ]; $$;
...
create index "index on FMAT.journal focus_from_path( path )" on FMAT.journal ( FMAT.focus_from_path( path ) );
create index "index on FMAT.journal topic_from_path( path )" on FMAT.journal ( FMAT.topic_from_path( path ) );
